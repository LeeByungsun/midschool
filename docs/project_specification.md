...
- 웹에서는 학교 검색을 통해 **학교/교육청 코드와 학교 종류(초등학교/중학교/고등학교)** 를 함께 저장할 수 있어야 합니다.
- Android도 동일하게 학교 검색 후 선택한 학교의 **학교/교육청 코드와 학교 종류(초등학교/중학교/고등학교)** 를 저장하고 조회 기준으로 사용해야 합니다.
...
- 모바일 앱은 타이머 종료 시 알림음/진동이 없어도 사용자가 알아볼 수 있도록 화면 배경 또는 타이머 카드 배경의 시각적 깜박임을 제공해야 합니다.
...
- **초기 설정 (`SetupActivity`)**
  ...
  - 유효한 학교·학년·반 저장 직후 메인 진입 전에 Analytics와 오류 진단 정보를 각각 선택할 수 있는 1회성 동의 단계를 표시한다. 기본값은 모두 해제이며, 거부해도 메인으로 진입하고 이후 설정에서 변경할 수 있다.
...
- **홈 화면 위젯 (`widget/`)**
  ...
  - 위젯 활성화 중에는 다음 로컬 자정 갱신 알람을 예약해 날짜 변경 브로드캐스트 누락 시에도 오늘/내일 시간표를 갱신
...
- 캐시/복구 전략
  ...
  - 로컬 캐시가 있으면 먼저 표시하고, 동시에 최신 요청으로 재검증
  - 최신 응답 데이터가 표시 중인 캐시와 다를 때만 화면 갱신
...
- **스터디 타이머 (`TimerView`, `TimerViewModel`)**
  ...
  - 타이머 종료 시 홈 타이머 영역과 상세 타이머 화면 배경을 깜박여 무음/무진동 상황에서도 완료 상태를 전달.

### 5.2 iOS 전용 UX/기능

- **홈 화면 위젯 (`SchoolHelperWidget`)**
  - WidgetKit 프레임워크 기반의 오늘/내일 시간표 위젯.
  - 작은/중간/큰 위젯에서 화면 크기에 맞게 오늘/내일 시간표를 요약 표시.
...
Android 앱은 선택적 Firebase Analytics/Crashlytics 관측 경계를 사용한다. 학교·교육청·학교 종류 코드, 기능 이용, 조회 성공/빈 결과/실패, 소요시간, 제한된 오류 코드를 기록하되 학교 이름·학년·반·검색어·URL·API 키·응답 원문은 전송하지 않는다. Analytics와 오류 진단은 독립 선택이며 기본값은 꺼져 있다. 최초의 유효한 학교·학년·반 저장 뒤에는 메인 화면 전에 1회성 선택 단계가 표시되며, 거부해도 앱 기능을 이용할 수 있고 Settings에서 언제든 변경할 수 있다. 수집 동작과 Play Data safety/개인정보처리방침은 출시 전에 일치 여부를 검토한다. 상세 이벤트 사전과 QA 절차는 `docs/android-firebase-observability.md`에 둔다.
